...
def trading_scan(strategy: str = "balanced") -> str:
    ...
    """
    from repryntt.trading.signal_scorer import score_signals, MIN_TRADE_SCORE

    # Auto-purge bundled tokens before scanning — keep the terminal clean
    try:
        import sqlite3 as _sq
        _db_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "tokens.db")
        if os.path.exists(_db_path):
            _conn = _sq.connect(_db_path, timeout=3)
            _bundled = _conn.execute(
                "SELECT address, token_name FROM tokens WHERE is_bundled = 1"
            ).fetchall()
            if _bundled:
                for _row in _bundled:
                    _conn.execute("DELETE FROM tokens WHERE address = ?", (_row[0],))
                _conn.commit()
                logger.info(f"[TRADING_SCAN] Auto-purged {len(_bundled)} bundled tokens")
            _conn.close()
    except Exception as _e:
        logger.debug(f"[TRADING_SCAN] Bundled purge failed (non-fatal): {_e}")
...
def build_trading_coldcall_prompt() -> Optional[str]:
    ...
    # Inject learning brief if the engine has data
    try:
        ...
        brief = TradingLearner(_eng).get_trading_brief(max_chars=1500)
        if brief:
            parts.append(f"\n---\n{brief}")
    except Exception:
        pass  # Learning engine not available — no brief, no problem
