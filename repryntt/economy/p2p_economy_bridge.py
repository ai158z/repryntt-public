...
# ═══════════════════════════════════════════════════════════════
#  GPU BENCHMARKING — uses canonical benchmark from proof_of_power
# ═══════════════════════════════════════════════════════════════

def measure_gpu_tflops() -> Tuple[float, Dict[str, Any]]:
    """Delegate to the canonical benchmark in proof_of_power.py.

    Returns (tflops, device_info) where tflops is the FP16 measurement
    (matching the blockchain node's compute_announce value).
    """
    try:
        from repryntt.economy.proof_of_power import measure_device_tflops
        tflops, pop_info = measure_device_tflops()
        # Adapt to the dict shape callers expect
        device_info = {
            "device_type": pop_info.get("device_type", "cpu"),
            "device_name": pop_info.get("device_name", "CPU"),
            "cuda_available": pop_info.get("device_type") == "cuda",
            "vram_mb": int(pop_info.get("memory_gb", 0) * 1024),
            "measured_tflops": tflops,
        }
        logger.info(f"⚡ GPU Benchmark: {tflops:.3f} TFLOPS ({device_info['device_name']})")
        return tflops, device_info
    except Exception as e:
        logger.warning(f"Canonical benchmark unavailable ({e}) — fallback")
        return 0.1, {
            "device_type": "cpu",
            "device_name": "CPU",
...
class P2PEconomyBridge:
    ...
    def _get_wallet_address(self) -> str:
        ...
        # Use the canonical node wallet — same identity as the blockchain node
        try:
            from repryntt.economy.node_wallet import get_node_wallet
            nw = get_node_wallet()
            if nw and nw.address:
                return nw.address
        except Exception:
            pass

        # Fallback: read from blockchain node directly
        if self.blockchain:
            addr = getattr(self.blockchain, 'node_address', None)
            if addr:
                return addr

        # Last resort: deterministic address from node ID
        return hashlib.sha3_256(self.p2p.node_id.encode()).hexdigest()[:40]
    ...
    async def _on_compute_announce(self, sender_id: str, payload: dict, ws, source_addr: str):
        ...
        async with self._compute_lock:
            ...
            # Bridge peer TFLOPS into blockchain's VRF candidate registry
            if wallet and tflops > 0 and self.blockchain is not None:
                contrib = getattr(self.blockchain, 'compute_contributors', None)
                if contrib is not None:
                    contrib[wallet] = tflops
    ...
    async def _call_local_llm(self, prompt: str, max_tokens: int = 512,
        ...
        """Call the local llama.cpp server for AI inference (cold-starts if needed)."""
        import aiohttp

        # Cold-start: spin up llama.cpp on demand if not running
        from repryntt.economy.llm_cold_start import llm_manager
        if not await llm_manager.ensure_ready_async():
            logger.error("Local LLM unavailable (cold-start failed)")
            return None
