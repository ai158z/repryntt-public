...
WORKSPACE_ROOT = Path(__file__).parent.parent / "agent_workspaces" / "jarvis"
...
# TTS defaults
DEFAULT_TTS_ENGINE = "google"  # "google" or "piper"
DEFAULT_TTS_VOICE = "en-US-Neural2-D"  # Deep, professional male voice
...
def create_video_project(title: str = "", genre: str = "documentary",
    ...
    return json.dumps({
        ...
        "project_dir": str(pdir.relative_to(Path(__file__).parent.parent)),
...
def write_screenplay(project_id: str = "", episode: int = 1,
    ...
    ep_data = {
        ...
        "screenplay_file": str(sp_file.relative_to(Path(__file__).parent.parent)),
    }
...
def create_shot_list(project_id: str = "", episode: int = 1,
    ...
    return json.dumps({
        ...
        "total_duration_sec": total_duration,
        "estimated_cost_usd": round(estimated_cost, 2),
        "shot_list_file": str(sl_file.relative_to(Path(__file__).parent.parent)),
...
def generate_video_clip(project_id: str = "", shot_id: str = "",
    ...
    if active_provider == "veo":
        ...
        if project_id and shot_id and shot_data and sl_file and sl:
            ...
            shot_data["clip_file"] = str(clip_path.relative_to(Path(__file__).parent.parent))
            with open(sl_file, "w") as f:
        ...
        return json.dumps({
            ...
            "clip_id": clip_id,
            "shot_id": shot_id,
            "file_path": str(clip_path.relative_to(Path(__file__).parent.parent)),
...
def generate_narration(project_id: str = "", episode: int = 1,
    ...
    for seg in narration_segments:
        ...
        if success and output_file.exists():
            ...
            audio_files.append({
                "scene": seg["scene"],
                "file": str(output_file.relative_to(Path(__file__).parent.parent)),
...
def _generate_google_tts(text: str, output_path: str, voice: str,
    ...
    api_key = config.get("google_gemini", {}).get("api_key", "")
    if not api_key:
        return False
...
def generate_music(project_id: str = "", episode: int = 1,
    ...
    return json.dumps({
        ...
        "episode": episode,
        "scene": scene,
        "file_path": str(output_file.relative_to(Path(__file__).parent.parent)),
        "duration_sec": round(duration, 2),
        "mood": mood,
        "genre": genre,
...
def assemble_edit(project_id: str = "", episode: int = 1,
    ...
    for shot in shot_list.get("shots", []):
        ...
        if clip_file:
            abs_path = base_dir / clip_file
            if abs_path.exists():
                clips.append(str(abs_path))
...
def qa_review_video(project_id: str = "", episode: int = 1,
    ...
    return json.dumps({
        ...
        "qa_report_file": str(qa_file.relative_to(Path(__file__).parent.parent)),
...
def render_final(project_id: str = "", episode: int = 1,
    ...
    return json.dumps({
        ...
        "project_id": project_id,
        "episode": episode,
        "final_file": str(final_path.relative_to(Path(__file__).parent.parent)),
...
def generate_thumbnail(project_id: str = "", episode: int = 1,
    ...
    api_key = config.get("google_gemini", {}).get("api_key", "")
    if not api_key:
        return json.dumps({"error": "No Gemini API key for image generation"})
    ...
    try:
        ...
        return json.dumps({"error": f"Image API failed: {str(e)[:200]}"})
    ...
    if not image_data:
        return json.dumps({"error": "Model returned no image"})
    ...
    return json.dumps({
        ...
        "project_id": project_id,
        "episode": episode,
        "file_path": str(thumb_file.relative_to(Path(__file__).parent.parent)),
        "size_kb": round(size_kb, 1),
...
# Gemini config for creative ideation
...
def _call_gemini_creative(system_prompt: str, user_prompt: str,
    ...
    """Call Gemini for creative text generation (screenplay/shot list ideation)."""
    import requests as _req
    ...
    resp = _req.post(_GEMINI_ENDPOINT, headers={
        "Content-Type": "application/json",
...
def auto_produce_video(prompt: str = "", duration: int = 30,
    ...
    vprov = _get_video_provider()
    active_provider = provider if provider in ("veo", "xai") else vprov["provider"]
...
{{
  "shots": [
    ...
    # ── Step 5: Assemble edit (if FFmpeg available and not skipped) ──
    assembly_result = None
    render_result = None

    if not skip_assembly and _ffmpeg_available() and len(actual_clips) > 1:
        _progress("assembly", "Assembling clips into final video...")
        try:
            assembly_result = json.loads(assemble_edit(
                project_id=project_id, episode=1,
                transitions=True, add_narration=False, add_music=False
            ))
            _progress("assembly", f"Assembled: {assembly_result.get('output_file', 'done')}")
        except Exception as e:
            _progress("assembly", f"Assembly warning: {str(e)[:100]}")
    ...
    result = {
        ...
        "files": {
            "clips": [str(f.relative_to(Path(__file__).parent.parent)) for f in clip_files],
            "renders": [str(f.relative_to(Path(__file__).parent.parent)) for f in render_files],
            "project_dir": str(pdir.relative_to(Path(__file__).parent.parent)),
        },
